...
export default function Sports({ myDot, homeData, newDate, sportsData }) {
  //   sportsData = 2;
  console.log(sportsData);
  return (
    <CarouselContainer id="sports" style={{ paddingTop: "90px" }}>
      <Header>
        Sports{" "}
        <i style={{ color: "#393939" }} className="fas fa-football-ball"></i>
      </Header>
      <Carousel
        cols={1}
        rows={sportsData % 2 === 0 ? 2 : 1}
        gap={20}
        loop
        showDots={sportsData > 2 ? true : false}
        dot={myDot}
        arrowLeft={
          sportsData > 2 ? (
      ...
      >
        {homeData
          .filter((about) => about.section === "sports")
          .map((topStories, key) => {
