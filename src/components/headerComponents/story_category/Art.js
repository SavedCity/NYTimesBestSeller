...
export default function Art({ myDot, homeData, newDate, artData }) {
  //   artData = 2;
  return (
    <CarouselContainer id="arts" style={{ paddingTop: "90px" }}>
      <Header>
        Arts <i style={{ color: "#393939" }} className="fas fa-palette"></i>
      </Header>
      <Carousel
        cols={1}
        rows={artData % 2 === 0 ? 2 : 1}
        gap={20}
        loop
        showDots={artData > 2 ? true : false}
        dot={myDot}
        arrowLeft={
          artData > 2 ? (
      ...
      >
        {homeData
          .filter((about) => about.section === "arts")
          .map((topStories, key) => {
