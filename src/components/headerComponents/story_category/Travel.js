...
export default function Travel({ myDot, homeData, newDate, travelData }) {
  //   travelData = 2;
  ...
  return (
    <CarouselContainer id="travel" style={{ paddingTop: "90px" }}>
      ...
      <Carousel
        cols={1}
        rows={travelData % 2 === 0 ? 2 : 1}
        gap={20}
        loop
        showDots={travelData > 2 ? true : false}
        dot={myDot}
        arrowLeft={
          travelData > 2 ? (
      ...
      >
        {homeData
          .filter((about) => about.section === "travel")
          .map((topStories, key) => {
