...
export default function NYRegion({ myDot, homeData, newDate, nyData }) {
  //   nyData = 7;
  return (
    <CarouselContainer id="ny" style={{ paddingTop: "30px" }}>
      <Header>
        N.Y. Region{" "}
        <i style={{ color: "#393939" }} className="fas fa-building"></i>
      </Header>
      <Carousel
        cols={1}
        rows={nyData % 2 === 0 ? 2 : 1}
      ...
      >
        {homeData
          .filter((about) => about.section === "nyregion")
          .map((topStories, key) => {
